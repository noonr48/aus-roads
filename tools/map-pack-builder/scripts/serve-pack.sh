...
PACK_VERSION="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["packVersion"])' "$DIST/latest.json")"
ZIP_REL="packs/$PACK_VERSION/pack.zip"
...
log "emulator base URL: http://10.0.2.2:${PORT}/"
log "Ctrl-C to stop."

# Serve from dist/ so /latest.json and /packs/<ver>/pack.zip resolve directly.
cd "$DIST"
